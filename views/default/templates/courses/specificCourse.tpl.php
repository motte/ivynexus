<script>
	function shareToSchool() {
		...
		xmlhttp.open("GET", "controllers/courses/specific/shareidea.php?a="+a+"&b="+b+"&c="+c+"&d="+d+"&e="+e+"&f="+f, true);
		xmlhttp.send();
	}
...
<div id="main">
	<div align="left" style="color:#196543; font-size:25px; margin-top: 20px;">{course_name}
		<span style="color: #444; margin-left:15px; font-size:17px;"><span id="chilirefresh{idea_id}">{chilis}</span>&nbsp<form id="ivychili" name="form" method="post"><input id="ivychili" title="Hotness Factor: {chilis} Chilis" type="button" onclick="addChili({course_id})" /></form></span>
	...
	<hr style="height: 1px; width: 80%; margin-top:20px; background: #fff; border: none; margin-bottom:20px;" />
	<div id="plus_sign" class="support" style="text-align:center;padding-left: 15px;" onclick="addSupport({course_id})">Support</div>
</div>
