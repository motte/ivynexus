...
class Post {
	...
	/**
	 * Friendly relative representation of when the post was created
	 */
	...
	public function __construct(Registry $registry, $id=0) {
		...
		$this->id = $id;
		if($this->id > 0) {
			$sql = "SELECT p.*, DATE_FORMAT(p.created, '%D %M %Y') as created_relative, pr.name as creator_name FROM posts p, profile pr WHERE pr.user_id=p.creator AND p.ID=" . $this->id;
