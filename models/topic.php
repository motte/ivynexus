...
class Topic {
	...
	public function __construct(Registry $registry, $id=0) {
		...
		$this->id = $id;
		if($this->id > 0) {
			$sql = "SELECT t.*, (SELECT COUNT(*) FROM posts po WHERE po.topic=t.ID) as posts, DATE_FORMAT(t.created, '%D %M %Y') as created_relative, p.name as creator_name FROM topics t, profile p WHERE p.user_id=t.creator AND t.ID=" . $this->id;
	...
	public function getPostsQuery() {
		$sql = "SELECT p.*, DATE_FORMAT(p.created, '%D %M %Y') as relative_created_post, pr.name as creator_relative_post FROM posts p, profile pr WHERE pr.user_id=p.creator AND p.topic=" . $this->id;
		return $sql;
	}
	...
	/**
	 * Conver the topic data to template tags
