...
class Events {
	...
	public function listEventsFuture($days) {
		$school = $this->registry->getObject('authenticate')->getProfile()->getSchool();
		$table = strtolower($school."_events");
		
		$sql = "SELECT event_name as name, event_description, event_start, event_starttime, event_end, event_endtime, chili, photo FROM $table WHERE event_start >= CURDATE() AND event_start <= DATE_ADD(CURDATE(), INTERVAL {$days} DAY)";
		$cache = $this->registry->getObject('db')->cacheQuery($sql);
		return $cache;
	}
	...
	/**
	 * List events in the future user that the user is invited to
	...
	/**
	 * List events in the future user that the user is attending
	 * @param int $user the user
	 * @return int database cache
	 */
	...
	/**
	 * List events in the future user that the user is not attending
	 * @param int $user the user
	 * @return int database cache
	 */
	...
	/**
	 * List events in the future user that the user is maybe attending
	 * @param int $user the user
	 * @return int database cache
	 */
