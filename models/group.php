...
class Group {
	...
	/**
	 * 'visible' representation of when the group was created
	 */
	private $createdRelative;
	...
	public function __construct(Registry $registry, $id=0) {
		...
		if($id > 0) {
			$this->id = $id;
			$sql = "SELECT g.*, DATE_FORMAT(g.created, '%D %M %Y') as created_relative, p.name as creator_name FROM groups g, profile p WHERE p.user_id=g.creator AND g.ID=" . $this->id;
			...
			if($this->registry->getObject('db')->numRows() == 1) {
				...
				$this->createdRelative = $data['created_relative'];
				$this->creator = $data['creator'];
	...
	/**
	 * Get a list of topics assigned to this group (I can paginate this later if I want to later)
	 * @return int (database cache)
	 */
	public function getTopics() {
		$sql = "SELECT t.*, (SELECT COUNT(*) FROM posts po WHERE po.topic=t.ID) as posts, DATE_FORMAT(t.created, '%D %M %Y') as created_relative, p.name as creator_name FROM topics t, profile p WHERE p.user_id=t.creator AND t.group=" . $this->id . " ORDER BY t.ID DESC";
