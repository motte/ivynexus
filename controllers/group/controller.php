...
class Groupcontroller {
		...
		if($this->registry->getObject('authenticate')->isLoggedIn()) {
			if(isset( $urlBits[1])) {
				...
				if($this->group->isValid() && $this->group->isActive()) {
					...
					if($this->group->getCreator() == $user || $gm->getApproved() || $this->group->getType() == 'private') {
						if(isset($urlBits[2])) {
							switch($urlBits[2]) {
								case 'create-topic':
									$this->createTopic();
						...
						// not sure if this works - if neither creator of group nor member but the group is public
						else {
							$this->viewGroup();
						}
					}

					// not sure if this works - if neither creator of group nor member and the group is private
					else {
						require_once(FRAMEWORK_PATH . 'controllers/group/membership.php');
						$membership = new Membershipcontroller($this->registry, $this->groupID);
						$membership->join();
						
						$this->cannotViewGroup();
					}					
				}
				else {
	...
	private function cannotViewGroup() {
		$this->group->toTags('group_');
		$this->registry->getObject('template')->buildFromTemplates('header.tpl.php', 'groups/not-member.tpl.php', 'footer.tpl.php');
	}
