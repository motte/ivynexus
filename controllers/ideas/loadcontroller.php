...
// This loads the idea posts on the ideas pages
	...
	$counter = mysqli_real_escape_string($_GET["c"]); //initial load has counter =0
	$first = mysqli_real_escape_string($_GET["d"]); //starts from 0, first time loading a set of posts, then adds minimum row value
	
//If not the first post
	if($first != '0') {
		$query = mysqli_query("SELECT * FROM ideas WHERE id BETWEEN $counter-26 AND $counter-1 ORDER BY id DESC LIMIT 25");
...
// if the first set of posts - mark it
		...
		$query = mysqli_query("SELECT * FROM ideas ORDER BY id DESC LIMIT 25");
