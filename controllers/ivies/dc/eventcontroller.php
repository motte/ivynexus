...
if($anonymous == 1) {
mysqli_query("INSERT INTO $school (poster_id, post, type, anonymous, name) VALUES('$user_id', '$post', '2', '1', '')");
echo '<p>'.$post.'</p><hr color="#fff" size="1" />';
}
else {
mysqli_query("INSERT INTO $school (poster_id, post, type, anonymous, name) VALUES('$user_id', '$post', '2', '0', '$poster')");
echo '<p><strong><a class="name" href="profile/view/'.$user_id.'">'.$poster.'</a></strong>: '.$post.'</p><hr color="#fff" size="1" />';
}
$eventtable = $school.'_events';
mysqli_query("INSERT INTO $eventtable (poster_id, event_name, event_description, event_start, event_starttime, event_end, event_endtime, photo) VALUES ('$user_id', '$event_name', '$event_description', '$event_start', '$event_starttime', '$event_end', '$event_endtime', '$actual_image_name')");
}
else
echo "failed";
}
