...
// Comment load when comment button pressed.
	...
	$poster_id = mysqli_real_escape_string($_GET["d"]);
	$school = mysqli_real_escape_string($_GET["e"]);
	$school_comments = $school."_comments";

	$query = mysqli_query("SELECT * FROM $school_comments WHERE ventpost_id=$post_id ORDER BY id ASC") or die(mysqli_error());
