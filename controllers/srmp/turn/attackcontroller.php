<?php
/*** This is the attack command that allows user to send soldiers into one of the surrounding enemy states ***/
	...
	$team_id = mysqli_real_escape_string($link,$_GET["a"]);
	$attackstatefrom = mysqli_real_escape_string($link,$_GET["b"]);
	$attackstateto = mysqli_real_escape_string($link,$_GET["c"]);
	$number_attacking_troops = mysqli_real_escape_string($link,$_GET["d"]);
	$turn = mysqli_real_escape_string($link, $_GET["e"]);
	$id = mysqli_real_escape_string($link,$_GET["f"]);
	
	$fromstate = substr($attackstatefrom, -4); 
    $fromstate = preg_replace('/\s+/', '', $fromstate);

    $tostate = substr($attackstateto, -4); 
    ...
    // This extracts the troop number from database on the user's attack origin state
    ...
    if($specific_troops>$number_attacking_troops){
    	$command = $number_attacking_troops.','.$attackstatefrom.';'.$attackstateto;
    	mysqli_query($link, "INSERT INTO srmp_ivy_commits (turn, type, team_id, user_id, commands, troops, fromstate, tostate) VALUES ('$turn', '2', '$team_id', '$id', '$command', '$number_attacking_troops', '$fromstate', '$tostate')") or die('error');
	    echo 'true';
	    //echo 'true';
	   
	}
	elseif($specific_troops<=$number_attacking_troops){
		echo 'not';
		//echo 'not';
