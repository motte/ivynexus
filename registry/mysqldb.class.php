...
class Mysqldb {
	...
	/**
	 ...
	 setActiveConnection($id) allows us to change this active connection
	 */
	...
	/**
	 Data which has been prepared and then cached for later usage, primarily wihin the template engine
	 */
	...
	/**
	 Reference to the registry object (connection to registry)
	 */
	private $registry;
	...
	/**
	 Change which database connection is actively used for the next operation (swap connections i.e. for looking up data from external source or authenticate against another system).
	...
	/**
	 ...
	 @return in the number of rows affected
	 */
	...
	/**
	 ...
	 @param String the condition (update operations)
	 @return bool
	 */
	...
	/**
	 Insert records into the academics database
	 @param nil the academics database table
	 @return bool
	 */
	public function createAcademics($table, $column, $input) {
	...
	/**
	 ...
	 @param String the table from which to remove rows
	...
	/**
	 Deconstruct the object (disconnect from all connections to all databases)
